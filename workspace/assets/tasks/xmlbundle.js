...
module.exports = function xmlbundle (grunt) {
	...
	grunt.registerTask('xmlbundle',
		...
		grunt.log.writeln('start xmlbundle');
		...
		var curThemeSelector = '//theme[@type = \'site\']';

		//Validate the theme
		var curTheme = themeDoc.find(curThemeSelector);

		if (curTheme.length == 0) {
			//ERROR
		}
		...
		var processColors = function () {
			var mergeColorsVariables = function () {
				...
				coreVariables.forEach(function (item) {
					var name = item.name();
					var overrideNode = themeDoc.find(curThemeSelector +'[1]/colors/type[@name=\'core-variables\']/' + name);
				...
				//Scan symbolic
				var symbolic = themeDoc.find(curThemeSelector +'[1]/colors/type[@name=\'symbolic\']/*');
		...
		//create final files
		//Writes vars
		fs.writeFile("css/theme/__vars.less", resultVar, function(err) {
			if(err) {
		...
		//Write class
		fs.writeFile("css/theme/__theme.less", resultTheme, function(err) {
			if(err) {
				return console.log(err);
			}
		}); 
