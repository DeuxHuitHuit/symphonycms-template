...
<!-- COMPONENT: form-field-number -->
	<xsl:template match="item[@section = 'form-field-number']" mode="form-field">
		...
		<xsl:param name="step" select="step" />
		...
		<xsl:variable name="has-min" select="string-length($min) != 0" />
		<xsl:variable name="has-max" select="string-length($max) != 0" />
		<xsl:variable name="has-step" select="string-length($step) != 0" />
		...
		<xsl:variable name="attr">
			...
			<add class="transition-form-field transition-form-field-number" />
			<add class="js-form-field js-form-field-number" />
			<xsl:copy-of select="$ext-attr"/>
			<add dev-component="form-field-number" />
		</xsl:variable>
