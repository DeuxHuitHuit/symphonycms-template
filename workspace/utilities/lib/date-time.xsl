...
<xsl:template name="format-month">
	...
	<xsl:param name="month-word">
		<xsl:choose>
			<xsl:when test="$lg = 'fr' and $month = 01">janvier</xsl:when>
			<xsl:when test="$lg = 'fr' and $month = 02">février</xsl:when>
			<xsl:when test="$lg = 'fr' and $month = 03">mars</xsl:when>
			<xsl:when test="$lg = 'fr' and $month = 04">avril</xsl:when>
			<xsl:when test="$lg = 'fr' and $month = 05">mai</xsl:when>
			<xsl:when test="$lg = 'fr' and $month = 06">juin</xsl:when>
			<xsl:when test="$lg = 'fr' and $month = 07">juillet</xsl:when>
			<xsl:when test="$lg = 'fr' and $month = 08">août</xsl:when>
			<xsl:when test="$lg = 'fr' and $month = 09">septembre</xsl:when>
			<xsl:when test="$lg = 'fr' and $month = 10">octobre</xsl:when>
			<xsl:when test="$lg = 'fr' and $month = 11">novembre</xsl:when>
			<xsl:when test="$lg = 'fr' and $month = 12">décembre</xsl:when>
	...
	<xsl:choose>
		<xsl:when test="$format = 'm'">
			<xsl:choose>
				<xsl:when test="$lg = 'fr' and ($month = 03 or $month = 05 or $month = 06 or $month = 08)">
					<xsl:value-of select="$month-word"/>
				</xsl:when>
				<xsl:when test="$lg = 'fr' and ($month = 01 or $month = 02 or $month = 07 or $month = 09)">
					<xsl:value-of select="substring($month-word, 1,4)" /><xsl:text>.</xsl:text>
				</xsl:when>
				<xsl:when test="$lg = 'en' and ($month = 09)">
					<xsl:value-of select="substring($month-word, 1,4)" /><xsl:text>.</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="substring($month-word, 1,3)" /><xsl:text>.</xsl:text>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:when>
		<xsl:when test="$format = 'M'">
...
<xsl:template name="format-day">
	...
	<xsl:param name="suffix">
		<xsl:choose>
			<xsl:when test="$lg = 'fr' and (substring($day,2) = 1) and not(substring($day,1,1) = 1)">er</xsl:when>
			<xsl:when test="$lg = 'fr' and (substring($day,2) = 2) and not(substring($day,1,1) = 1)">e</xsl:when>
			<xsl:when test="$lg = 'fr' and (substring($day,2) = 3) and not(substring($day,1,1) = 1)">e</xsl:when>
			...
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="$lg = 'fr'">e</xsl:when>
